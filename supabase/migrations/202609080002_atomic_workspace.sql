-- Atomic snapshots avoid partial imports and lost updates between ordinary app sessions.
-- SECURITY INVOKER: these functions have precisely the caller's privileges; RLS applies.
begin;
create function public.load_workspace() returns jsonb
language sql stable security invoker set search_path = '' as $$
 select jsonb_build_object(
  ...
  'data',jsonb_build_object(
   ...
   'transactions',coalesce((select jsonb_agg(jsonb_build_object('id',id,'type',type,'amount',amount,'categoryId',category_id,'date',to_char(date,'YYYY-MM-DD'),'note',note) order by date desc,id) from public.transactions where user_id=auth.uid()),'[]'::jsonb),
...
$$;
create function public.save_workspace(payload jsonb, expected_revision bigint, import_hash text default null) returns jsonb
...
begin
 ...
 select (x->>'id')::uuid,owner,x->>'type',(x->>'amount')::bigint,(x->>'categoryId')::uuid,(x->>'date')::date,x->>'note' from jsonb_array_elements(payload->'transactions') x
 on conflict(id) do update set type=excluded.type,amount=excluded.amount,category_id=excluded.category_id,date=excluded.date,note=excluded.note;
...
revoke all on function public.save_workspace(jsonb,bigint,text) from public,anon;
grant execute on function public.load_workspace() to authenticated;
grant execute on function public.save_workspace(jsonb,bigint,text) to authenticated;
commit;
